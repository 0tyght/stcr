...
const [companyId, name = "Express gateway", allowedOvenId = ""] = process.argv.slice(2);
...
if (!new Set(["gr", "ttn"]).has(companyId) || pepper.length < 32) {
  ...
  console.error("node backend/tools/create-api-key.mjs <gr|ttn> [name] [ovenId]");
  process.exit(1);
}
...
console.log("Generated API key (store in the Express secret environment; shown only now):");
console.log(apiKey);
