...
const nodes = [
  ...
  { id: mqttTabId,     type: "tab", label: "02 รับข้อมูล MQTT โรงงาน", disabled: false, info: "รับข้อมูลจริงจาก MQTT โรงงาน เขียนลง MySQL โดยตรง" },

  // ── tab 01: HTTP API ───────────────────────────────────────────────────────
  { id: "stcr-platform-note", type: "comment", z: platformTabId,
    name: "ฐานข้อมูลจริงและ API สำหรับเว็บไซต์",
    info: "เมื่อไม่ได้รับข้อมูลจริง ระบบจะแสดงสถานะขาดการเชื่อมต่อ",
    x: 350, y: 40, wires: [] },

  ...endpoints.map(([method, url, name], i) => ({
    id: `stcr-http-${i + 1}`, type: "http in", z: platformTabId,
    name, url, method, upload: false, swaggerDoc: "",
    x: 180, y: 100 + i * 42, wires: [[routerId]],
  })),

  fnNode({
    id: routerId, z: platformTabId,
    name: "จัดเส้นทาง STCR API และอ่านเขียน MariaDB",
    func: router, x: 570, y: 440, wires: [[responseId]],
    libs: [{ var: "mysql", module: "mysql2/promise" }, { var: "crypto", module: "crypto" }],
  }),
  { id: responseId, type: "http response", z: platformTabId,
    name: "ส่งผลลัพธ์ HTTP กลับเว็บไซต์",
    statusCode: "", headers: {}, x: 880, y: 440, wires: [] },

  // ── tab 02: MQTT pipeline ─────────────────────────────────────────────────
  // subscriber → adapter → [inspection, db-writer]
  // ไม่มี http request node เพราะเขียน MySQL โดยตรง
  { id: "stcr-factory-mqtt-note", type: "comment", z: mqttTabId,
    name: "รับข้อมูลจริงจาก MQTT ของโรงงาน TTN",
    info: "รับ Topic test และ sensor เขียน MySQL โดยตรง ไม่ผ่าน HTTP self-call",
    x: 320, y: 60, wires: [] },

  fnNode({
    id: "stcr-factory-mqtt-subscriber", z: mqttTabId,
    name: "รับ MQTT โรงงาน: test และ sensor",
    func: "return null;", initialize: subscriberInit, finalize: subscriberFin,
    libs: [
      { var: "mqtt",  module: "mqtt" },
      { var: "mysql", module: "mysql2/promise" },
    ],
    x: 230, y: 160, wires: [["stcr-factory-mqtt-adapter"]],
  }),

  fnNode({
    id: "stcr-factory-mqtt-adapter", z: mqttTabId,
    name: "ตรวจสอบและแปลงข้อมูล MQTT",
    func: adapter, outputs: 1, libs: [],
    x: 560, y: 160,
    wires: [["stcr-factory-mqtt-inspection", "stcr-factory-mqtt-db-writer"]],
  }),

  fnNode({
    id: "stcr-factory-mqtt-inspection", z: mqttTabId,
    name: "แสดงผลตรวจสอบข้อมูล MQTT",
    func: inspection, x: 860, y: 120, wires: [[]],
  }),

  fnNode({
    id: "stcr-factory-mqtt-db-writer", z: mqttTabId,
    name: "เขียนข้อมูล MQTT ลง MySQL โดยตรง",
    func: dbWriter, x: 860, y: 200, wires: [[]],
    libs: [
      { var: "mysql",  module: "mysql2/promise" },
